...
bool MyBaseDialog::nativeEvent(const QByteArray &event, void *message1, long *result)
{
    ...
    {
    case WM_NCHITTEST://当光标移动到窗体，或鼠标按下，释放时，发送WM_NCHITTEST消息
...
void MyBaseDialog::paintEvent(QPaintEvent */*event*/)
{
    ...
    p.drawPixmap(0,0,QPixmap(":/image/bg_mainjpt.png").scaled(this->size(),Qt::IgnoreAspectRatio,Qt::SmoothTransformation)); // 绘制不规则背景
