...
{
    if (!self) {
        QMutex mutex;//QMutex目的是保护一个对象，同一时间只能有一个线程访问
...
{
    int fontId = QFontDatabase::addApplicationFont(":/image/fontawesome-webfont.ttf");//添加字库
...
//设置QLabel图标   QLabel图标, text , size 大小
